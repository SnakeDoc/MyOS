...
void ExecService(func_p_t p, int arg) {
  ...
  MyMemcpy((char*)addr, (char*)p, PAGE_SIZE);

  j = (int*)(addr + PAGE_SIZE - 4);
  *j = arg;
  j--;
  *j = 0; // end of arguments

  k = (int*)((addr + PAGE_SIZE) - (sizeof(int) * 2) - sizeof(trapframe_t));
  MyMemcpy((char*)k, (char*)pcb[run_pid].trapframe_p, sizeof(trapframe_t));
  pcb[run_pid].trapframe_p = (trapframe_t*)k;
  pcb[run_pid].trapframe_p->eip = (int)addr;
}
