...
void sys_signal(int sig, func_p_t func) {
    asm("movl %0, %%eax; // SYS_SIGNAL #48
         movl %1, %%ebx; // signal
         movl %2, %%ecx; // handler
         int $128"
         :
         : "g" (SYS_SIGNAL), "g" (sig), "g" ((int)func)
         : "eax", "ebx", "ecx"
    );
}

void sys_exit(int exit_code) {
	asm("movl %0, %%eax; // SYS_EXIT #1
...
int sys_waitchild(int *exit_code_p) {
	int child_pid;
	asm("movl %1, %%eax; // SYS_WAITCHILD #7
	     movl %2, %%ebx;
	     int $128;
	     movl %%ecx, %0"
	     : "=g" (child_pid)
...
void sys_exec(func_p_t p, int arg) {
  asm("movl %0, %%eax; // SYS_EXEC
       movl %1, %%ebx; // func_p_t p
       movl %2, %%ecx; // which
       int $128"
       :
