...
void ExitService(int exit_code){	//as child calls sys_exit()
    ...
    if(pcb[ppid].state != WAITCHILD) {
	    pcb[run_pid].state = ZOMBIE;
		  run_pid = -1;
      set_cr3(OS_TT); // restore MMU to OS_TT
...
void WaitchildService(int *exit_code_p, int *child_pid_p){ //parent requests
    ...
    i = get_cr3();
    set_cr3(pcb[child_pid].TT); // we need child trapframe addr
    *exit_code_p = pcb[child_pid].trapframe_p->ebx;
    set_cr3(i); // restore MMU to OS_TT
...
void ExecService(func_p_t p, int arg) {
  int i, entry, *q;
  trapframe_t *o_TF_p;
  int TT = 0; // Address Translation Table
  int IT = 1; // Instruction Table
  int ST = 2; // Stack Table
  int IP = 3; // Instruction Page
  int SP = 4; // Stack Page

  for (i = 0; i < 5; i++) { // initialize 5 pages
    pcb[run_pid].page[i] = DeQ(&page_q);
    if (pcb[run_pid].page[i] == -1) {
      cons_printf("Kernel Panic: No available DRAM pages!\n\r");
      return;
    }
  }

  // Build TT
  pcb[run_pid].TT = page_addr(pcb[run_pid].page[TT]); // address of Trans Table

  MyBzero((char *)pcb[run_pid].TT, sizeof(PAGE_SIZE));
  MyMemcpy((char *)pcb[run_pid].TT, (char*)OS_TT, sizeof(int[4])); // copy 4 bytes from OS_TT
  ...
  // build IP
                   // address of instruction page    // copy instruction
  ...
  *q = 0; // return addr is NUL
  ...
  // copy old TF into SP
  MyMemcpy((char *)((int)q - sizeof(trapframe_t)), (char *)o_TF_p, sizeof(trapframe_t));
